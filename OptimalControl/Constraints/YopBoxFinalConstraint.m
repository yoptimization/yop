...
% -------------------------------------------------------------------------
classdef YopBoxFinalConstraint < YopConstraint

    methods
        function obj = YopBoxFinalConstraint(constraint)
            
            [isMember, memberNumber] = YopBoxFinalConstraint.isMember(constraint);
            assert(isMember);
            
            pattern = YopBoxFinalConstraint.getMemberPattern(memberNumber);
    ...
    methods (Static)        
        ...
        function patterns = memberPatterns()
            ...
            p1 = YopConstraintPattern({'numeric' '<=' 'BoxFinalTimepoint' '<=' 'numeric'}, ...
                @(c) c{3}.Expression, @(c) c{5}, @(c) c{1});
            
            p2 = YopConstraintPattern({'numeric' '==' 'BoxFinalTimepoint'}, ...
                @(c) c{3}.Expression, @(c) c{1}, @(c) c{1});
            
            p3 = YopConstraintPattern({'numeric' '<=' 'BoxFinalTimepoint'}, ...
                @(c) c{3}.Expression, @(c) inf(size(c{3}.Expression)), @(c) c{1});
            
            p4 = YopConstraintPattern({'numeric' '>=' 'BoxFinalTimepoint'}, ...
        ...
        function pattern = getMemberPattern(patternNumber)
            patterns = YopBoxFinalConstraint.memberPatterns;
