...
% -------------------------------------------------------------------------
classdef YopBoxInitialConstraint < YopConstraint

    methods
        function obj = YopBoxInitialConstraint(constraint)
            
            [isMember, memberNumber] = YopBoxInitialConstraint.isMember(constraint);
            assert(isMember);
            
            pattern = YopBoxInitialConstraint.getMemberPattern(memberNumber);
    ...
    methods (Static)        
        ...
        function patterns = memberPatterns()
            ...
            p1 = YopConstraintPattern({'numeric' '<=' 'BoxInitialTimepoint' '<=' 'numeric'}, ...
                @(c) c{3}.Expression, @(c) c{5}, @(c) c{1});
            
            p2 = YopConstraintPattern({'numeric' '==' 'BoxInitialTimepoint'}, ...
                @(c) c{3}.Expression, @(c) c{1}, @(c) c{1});
            
            p3 = YopConstraintPattern({'numeric' '<=' 'BoxInitialTimepoint'}, ...
                @(c) c{3}.Expression, @(c) inf(size(c{3}.Expression)), @(c) c{1});
            
            p4 = YopConstraintPattern({'numeric' '>=' 'BoxInitialTimepoint'}, ...
        ...
        function pattern = getMemberPattern(patternNumber)
            patterns = YopBoxInitialConstraint.memberPatterns;
