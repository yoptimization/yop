...
% -------------------------------------------------------------------------
classdef YopBoundaryFinalConstraint < YopNonlinearConstraint

    methods
        function obj = YopBoundaryFinalConstraint(constraint, input, descaling)            
            [isMember, memberNumber] = YopBoundaryFinalConstraint.isMember(constraint);
            assert(isMember);
            
            pattern = YopBoundaryFinalConstraint.getMemberPattern(memberNumber);
    ...
    methods (Static)        
        ...
        function patterns = memberPatterns()
            ...
            p1 = YopConstraintPattern({'numeric' '<=' 'FinalBoundary' '<=' 'numeric'}, ...
                @(c) c{3}.Expression, @(c) c{5}, @(c) c{1});
            
            p2 = YopConstraintPattern({'numeric' '==' 'FinalBoundary'}, ...
                @(c) c{3}.Expression, @(c) c{1}, @(c) c{1});
            
            p3 = YopConstraintPattern({'numeric' '<=' 'FinalBoundary'}, ...
                @(c) c{3}.Expression, @(c) inf(size(c{3}.Expression)), @(c) c{1});
            
            p4 = YopConstraintPattern({'numeric' '>=' 'FinalBoundary'}, ...
        ...
        function pattern = getMemberPattern(patternNumber)
            patterns = YopBoundaryFinalConstraint.memberPatterns;
